...
async function dashboard(routeParams = new URLSearchParams()) {
  ...
  const bindDashboardSelection = (rows) => {
    ...
    document.querySelector("[data-action=apply-bulk]")?.addEventListener("click", async () => {
      ...
      const priorityValue = String(document.querySelector("[data-bulk-priority]")?.value || "");
      const targetDateValue = String(document.querySelector("[data-bulk-target]")?.value || "");
      const patch = {};
      if (statusValue) patch.status = statusValue;
      if (priorityValue) patch.priority = priorityValue;
      if (targetDateValue) patch.target_date = targetDateValue;
      if (!Object.keys(patch).length) {
        toast("Choose at least one field to update");
        return;
      }
      await Promise.all(
...
function renderTab() {
  ...
  if (state.tab === "overview")
    node.innerHTML = `<div class="section-toolbar"><div><p class="eyebrow">Required coverage</p><h3>Twelve areas of investigation</h3></div><span class="subtle">Select an area to write its narrative</span></div><div class="area-grid">${Object.entries(
      ...
      .map(
        ([key, name], i) =>
          `<div class="area-card ${esc(c.areas[key].status)}" data-area="${esc(key)}"><span class="area-num">${String(i + 1).padStart(2, "0")}</span><div><strong>${esc(name)}</strong><small>${label(c.areas[key].status)}</small></div></div>`,
      )
      .join("")}</div>`;
    ...
    node.innerHTML = records(
      ...
      "add-phs-change",
      (x) =>
        `<div class="record"><div><h4>${esc(x.field_label)} <span class="subtle">${esc(x.id)}</span></h4><p><strong>Prior:</strong> ${esc(x.prior_value || "Not entered")}<br><strong>Current:</strong> ${esc(x.current_value || "Not entered")}<br><strong>Reported:</strong> ${esc(x.reported_at || "Not entered")}<br>${esc(x.disposition || "No disposition yet")}</p></div><div class="record-actions">${status(x.disposition ? "recorded" : "pending")}<button class="quiet" data-edit-phs-change="${esc(x.id)}">Update</button></div></div>`,
    );
...
async function alerts() {
  ...
  app.innerHTML = `<div class="page-head"><div><p class="eyebrow">Operational awareness</p><h1>Case alerts</h1><p>Review submissions, approvals, and correction requests stay visible here.</p></div></div><section class="panel"><div class="alert-list">${notices.map((notice) => `<article class="alert-item ${notice.read_at ? "read" : ""}"><div><p class="eyebrow">${esc(label(notice.kind))}</p><h3>${esc(notice.message)}</h3><span class="subtle">${new Date(notice.created_at).toLocaleString()}</span></div><div class="record-actions">${notice.case_id ? `<button class="quiet" data-open-case="${esc(notice.case_id)}">Open case</button>` : ""}${!notice.read_at ? `<button class="secondary" data-read-alert="${esc(notice.id)}">Mark read</button>` : ""}</div></article>`).join("") || '<div class="empty"><strong>No alerts</strong>Review workflow updates will appear here.</div>'}</div></section>`;
